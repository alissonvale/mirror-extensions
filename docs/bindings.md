...
**Suggested personas.** Any persona that talks about money:
`treasurer`, `cfo`, `founder`. Localized names (e.g. `tesoureira` in
Portuguese) work equally well — the manifest's `suggested_personas`
field is informational; the user picks. See
[`persona-recipes.md`](persona-recipes.md) for ready-to-use briefings
...
```bash
python -m memory ext finances unbind financial_summary --persona treasurer
```
...
[PF] BankA Checking Personal — R$ 12.345,67 (on 2026-05-09)
[PF] BankB Savings — R$ 94.123,10 (on 2026-05-10)
